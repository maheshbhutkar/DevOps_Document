...
wget -nv --spider --post-file=$FEATURE_NAME.xml --header='Content-type: application/xml; charset=ISO-8859-1' --no-proxy --auth-no-challenge --http-user=$USER_NAME --http-password=****** http://localhost:1080/view/FeaturesCareIdWebApp/createItem?name=$FEATURE_NAME > /dev/null
rm $FEATURE_NAME.xml
